    ...
    pub fn validate(&self) -> Result<(), ConfigError> {
        for (name, service) in &self.services {
            ...
            if let Some(ref org) = service.organization {
                let param = org.organization_param.as_deref().ok_or_else(|| ConfigError::InvalidRoute {
                    service: name.clone(),
...
fn reject_duplicate_path_methods(service: &str, svc: &ServiceConfig) -> Result<(), ConfigError> {
    ...
    for scope in [svc.public.as_ref(), svc.user.as_ref(), svc.organization.as_ref()].into_iter().flatten() {
...
pub fn join_route_prefix(prefix: &str, path: &str) -> Result<String, String> {
    ...
    if path != "/" && !path.starts_with('/') {
        return Err(format!("route path '{}' must be '/' or start with '/'", path));
    }
...
fn validate_scope_routes(
    ...
    for route in &scope.routes {
        ...
        if matches!(route.methods_form, MethodsForm::Mixed) {
            return Err(ConfigError::InvalidRoute {
                service: service.to_string(),
                reason: format!("{} route '{}' must not mix methods list and map", scope_name, route.path),
            });
        }
        ...
        if let Some(param) = organization_param {
            ...
            if !expanded.contains(&needle) {
                return Err(ConfigError::InvalidRoute {
                    service: service.to_string(),
                    reason: format!("organization route '{}' must include path param {{{}}}", expanded, param),
                });
            }
        }
        match &route.methods_form {
            MethodsForm::Nested(entries) => {
                if entries.is_empty() {
                    return Err(ConfigError::InvalidRoute {
                        service: service.to_string(),
                        reason: format!("{} route '{}' methods map must not be empty", scope_name, route.path),
                    });
                }
                for (method, spec) in entries {
                    ...
                    validate_required_scopes(service, scope_name, &route.path, spec.required_scopes.as_deref())?;
                    validate_rate_limit(service, scope_name, &route.path, spec.rate_limit.as_ref())?;
                }
            }
            MethodsForm::List => {
...
fn validate_required_scopes(
    ...
    if labels.is_empty() {
        return Err(ConfigError::InvalidRoute {
            service: service.to_string(),
            reason: format!("{} route '{}' required_scopes must be omitted or a non-empty list", scope_name, path),
        });
    }
    if labels.len() > MAX_SCOPE_COUNT {
        return Err(ConfigError::InvalidRoute {
            service: service.to_string(),
            reason: format!("{} route '{}' required_scopes has more than {} labels", scope_name, path, MAX_SCOPE_COUNT),
        });
    }
    ...
    for label in labels {
        ...
        if seen.insert(label, ()).is_some() {
            return Err(ConfigError::InvalidRoute {
                service: service.to_string(),
                reason: format!("{} route '{}' required_scopes has duplicate label '{}'", scope_name, path, label),
            });
        }
    }
    Ok(())
...
fn validate_rate_limit(
    ...
    if cfg.window_seconds == 0 {
        return Err(ConfigError::InvalidRoute {
            service: service.to_string(),
            reason: format!("{} route '{}' rate_limit.window_seconds must be > 0", scope_name, path),
        });
    }
    if let Some(ref by) = cfg.by {
        if !matches!(by.as_str(), "ip" | "user" | "member") {
            return Err(ConfigError::InvalidRoute {
                service: service.to_string(),
                reason: format!("{} route '{}' rate_limit.by must be ip, user, or member", scope_name, path),
            });
        }
    }
    Ok(())
...
mod tests {
    ...
    fn join_prefix_root_path() {
        assert_eq!(join_route_prefix("/api/organizations", "/").unwrap(), "/api/organizations");
    }
    ...
    fn prepare_expands_prefix() {
        ...
        assert_eq!(prepared.user.as_ref().unwrap().routes[0].path, "/api/organizations");
    }
    ...
    fn required_scopes_reject_empty_and_bad() {
        for labels in [vec![], vec!["Widgets:read".into()], vec!["bad/scope".into()], vec!["ok".into(), "ok".into()]] {
            let mut r = route("/api/widgets", &["GET"]);
            r.required_scopes = Some(labels.clone());
            let mut services = HashMap::new();
            services.insert("w".into(), user_service(vec![r]));
            assert!(Config { services }.validate().is_err(), "expected error for {labels:?}");
        }
    }
    ...
    fn rate_limit_object_must_be_positive() {
        let mut r = route("/api/widgets", &["GET"]);
        r.rate_limit = Some(RouteRateLimit::Limit(RateLimitConfig { requests: 0, window_seconds: 60, by: None }));
        let mut services = HashMap::new();
        services.insert("w".into(), user_service(vec![r]));
    ...
    fn scopes_intersect_nonempty() {
        assert!(scopes_intersect(&["a".into(), "b".into()], &["b".into(), "c".into()]));
    ...
    fn flat_methods_list_still_validates() {
        ...
        assert_eq!(r.required_scopes.as_deref(), Some(["org:read".to_string()].as_slice()));
    ...
    fn nested_get_post_different_scopes_expands_to_two_routes() {
        ...
        let prepared = user_service(vec![r]).prepare_for_registry("w").expect("expand nested methods");
        let routes = &prepared.user.as_ref().unwrap().routes;
        assert_eq!(routes.len(), 2);
        let get = routes.iter().find(|rt| rt.methods == ["GET"]).expect("GET row");
        assert_eq!(get.path, "/features");
        assert_eq!(get.required_scopes.as_deref(), Some(["org:read".to_string()].as_slice()));
        assert!(get.rate_limit.is_none());
        assert!(matches!(get.methods_form, MethodsForm::List));
        let post = routes.iter().find(|rt| rt.methods == ["POST"]).expect("POST row");
        assert_eq!(post.required_scopes.as_deref(), Some(["org:write".to_string()].as_slice()));
    ...
    fn empty_nested_methods_map_fails() {
        ...
        let err = Config { services }.validate().unwrap_err();
        let msg = err.to_string();
        assert!(msg.contains("methods map must not be empty") || msg.contains("no methods"), "expected empty map error, got {msg}");
    }
    ...
    fn nested_method_empty_body_ok() {
        ...
        let prepared = user_service(vec![r]).prepare_for_registry("w").expect("empty method body is valid");
        let routes = &prepared.user.as_ref().unwrap().routes;
        assert_eq!(routes.len(), 2);
        for rt in routes {
    ...
    fn duplicate_path_method_after_expand_fails() {
        ...
        let err = user_service(vec![nested, flat]).prepare_for_registry("w").unwrap_err();
        let msg = err.to_string();
        assert!(msg.contains("duplicate"), "expected duplicate path+method, got {msg}");
    }
    ...
    fn yaml_nested_empty_body_ok() {
        ...
        let prepared = user_service(vec![r]).prepare_for_registry("w").expect("yaml empty method body is valid");
        let routes = &prepared.user.as_ref().unwrap().routes;
        assert_eq!(routes.len(), 2);
        for rt in routes {
            assert!(rt.required_scopes.is_none());
