...
mod tests {
    ...
    fn unknown_fields_do_not_deserialize() {
        let route_err = serde_json::from_str::<RouteConfig>(
            r#"{"path":"/x","methods":["GET"],"transform":{"path":"/y"}}"#,
        )
        .unwrap_err();
        assert!(
            route_err.to_string().contains("unknown field"),
            "{route_err}"
        );
        let scope_err = serde_yaml::from_str::<ScopeConfig>(
            "routes: []\norganization_param: organization_id\n",
        )
        .unwrap_err();
        assert!(
            scope_err.to_string().contains("unknown field"),
            "{scope_err}"
        );
    }
}
