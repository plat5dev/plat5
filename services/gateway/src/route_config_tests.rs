...
mod tests {
    ...
    fn rate_limit_false_and_object() {
        ...
        // `by` must deserialize (not silently drop) so validate can 422.
        let with_by: RouteRateLimit =
    ...
    fn nested_rate_limit_by_is_rejected() {
        for by in ["key", "member", "org", "ip"] {
            let r = parse_route(&format!(
                r#"{{"path":"/features","methods":{{"POST":{{"rate_limit":{{"requests":10,"window_seconds":60,"by":"{by}"}}}}}}}}"#
            ));
            let err = config_with(r).validate().unwrap_err();
            let msg = err.to_string();
            assert!(
