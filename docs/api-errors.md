...
| Limiter | When | Key |
|---------|------|-----|
| Per-route (admitted) | After match + admission (JWT and API key). Omitted `rate_limit` inherits `RATE_LIMIT_REQUESTS` / `RATE_LIMIT_WINDOW_SECONDS` (never silent unlimited). `rate_limit: false` opts out. `{requests, window_seconds}` overrides. | Route scope: `public`→ip, `user`→user, `organization`→org (org id, including SA/member keys) |
